...
const ItemsPopup: React.FC<ItemsPopupProps> = ({ setPopup, selectedItems, onItemsSelected }) => {
  const menuItems: Item[] = [
    ...
    { id: 7, name: "Paneer Butter Masala", price: 349, category: "Main Course" },
  ...
  return (
    <div className={`${styles.popup} ${styles.itemsPopup}`}>
          ...
          <div key={category} className={styles.category}>
            ...
            <div className={styles.itemsList}>
              {menuItems
                ...
                .map((item) => (
                  <div key={item.id} className={styles.itemCard} onClick={() => toggleItem(item)}>
                    <div className={styles.itemInfo}>
                      <div className={styles.itemName}>{item.name}</div>
                      <div className={styles.itemPrice}>₹{item.price}</div>
                    </div>
                    <div
                      className={`${styles.checkbox} ${selected.some((i) => i.id === item.id) ? styles.checked : ""}`}
                    >
                      {selected.some((i) => i.id === item.id) && <Check size={16} />}
                    </div>
                  </div>
                ))}
            </div>
      ...
      <div className={styles.popupFooter}>
        <div className={styles.selectedCount}>{selected.length} items selected</div>
